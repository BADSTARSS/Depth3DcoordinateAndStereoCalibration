...
# Camera Object
cap = cv2.VideoCapture(1)  # Number According to Camera
...
while True:
    ...
    if len(bbox) == 0:
        ...
        if D < 250 and D != 0:
            frame = cv2.putText(frame, "!!MOVE AWAY!!", (100, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, [0, 0, 255], 4)

        frame = cv2.putText(frame, str(D / 10) + " cm", (300, 50), cv2.FONT_HERSHEY_SIMPLEX,
                            1, (255, 0, 0), 2, cv2.LINE_AA)

        cv2.imshow('objdetect_3d :', frame)
